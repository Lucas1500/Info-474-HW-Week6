...
function makeScatterPlot(csvData) {
    ...
    data_1980 = data.filter(function(d){return d["year"]==1980})

    let fertility_rate_data = data_1980.map((row) => parseFloat(row["fertility"]));
    let life_expectancy_data = data_1980.map((row) => parseFloat(row["life_expectancy"]));
...
function plotData(map) {
    let pop_data = data_1980.map((row) => +row["population"]);
    ...
    let large_country = data_1980.filter(function(d) {return +d['population'] > 100000000 })
